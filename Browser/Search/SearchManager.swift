...
class SearchManager {
  ...
  var matchedWebsiteSearcher: any WebsiteSearcher {
    SearchEngine.allSearchers.first(where: { $0.title.lowercased().hasPrefix(searchText.lowercased()) }) ?? SearchEngine.google.searcher
  }
  var isUsingWebsiteSearcher: Bool = false
  var activeWebsiteSearcher: any WebsiteSearcher = SearchEngine.google.searcher
  ...
  func handleTab() -> KeyPress.Result {
    withAnimation(.browserDefault) {
      ...
      } else if matchedWebsiteSearcher.title != SearchEngine.google.searcher.title {
